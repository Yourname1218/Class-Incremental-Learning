...
class CIFAR10(data.Dataset):
    ...
    def RandomPercentage(self, data, targets, index):
        ...
        # 將數據轉換為 numpy 數組以便處理
        data = np.array(data)
        targets = np.array(targets, dtype=np.int64)  # 確保標籤是 int64 類型
        ...
        for i in index:
            ...
            if len(data_tmp) == 0:
                data_tmp = data[idx]
                targets_tmp = targets[idx]
            else:
                data_tmp = np.concatenate((data_tmp, data[idx]), axis=0)
                targets_tmp = np.concatenate((targets_tmp, targets[idx]))
